...
import {type LocationQueryRaw, type RouteLocationRaw, type RouteParams} from "vue-router";
...
export const useCarContextStore = defineStore('car-context', () => {
  ...
  const headers = computed(() => {
    ...
    return {
      ...useContextHeader(provider.value),
      ...useTenantHeader(tenantId.value),
    }
  })
...
export const useCarListingStore = defineStore('car-listing', () => {
  ...
  const fetch = useFetch(
    ...
    {
      query,
      // TODO:: expose a type from @antify/context to fix this type error
      headers,
      watch: false,
      immediate: false
    }
  ...
  return {
    ...
    skeleton: useUiClient().utils.createSkeleton(fetch.pending),
    error: fetch.error,
